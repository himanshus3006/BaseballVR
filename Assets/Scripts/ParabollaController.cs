...
//Config   
    ...
    public GameObject[] ParabolaPoints;
    public GameObject ParabolaRoot;
  
    public bool travel = true;
    internal bool nextParbola = false;
    protected float travelTime = float.MaxValue;
    
    protected ParabolaFly gizmo;
    protected ParabolaFly parabolaFly;


    // just for reference , draws an arrow of dotted points along which the ball follows
    void OnDrawGizmos()
    {
        if (gizmo == null)
        {
    ...
    void Update()
    {
        // select random path for ball to traverse
        ParabolaRoot = ParabolaPoints[Random.Range(0, ParabolaPoints.Length)];
        nextParbola = false;

        // checks if next point can be plotted 
        if (travel && parabolaFly != null && travelTime < parabolaFly.GetDuration())
        {
            int parabolaIndexStart;
            int parabolaIndexNext;
            parabolaFly.GetParabolaIndexAtTime(travelTime, out parabolaIndexStart);
            travelTime += Time.deltaTime;
            parabolaFly.GetParabolaIndexAtTime(travelTime, out parabolaIndexNext);

            transform.position = parabolaFly.GetPositionAtTime(travelTime);

            if (parabolaIndexStart != parabolaIndexNext)
                nextParbola = true; 
        }
        else if (travel && parabolaFly != null && travelTime > parabolaFly.GetDuration())
        {
    ...
    // makes ball follow the along the points
    ...
    // sends ray to check distance betweent points 
